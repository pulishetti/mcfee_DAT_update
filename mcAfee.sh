...
install_dir=$(dirname `which uvscan`) || exit_error "uvscan not installed"
...
# Function to parse avvdat and return, via stdout, the
# contents of a specified section. Requires the avvdat
...
# Function to download a specified file from update.nai.com via http
# $1 - Path on http server
# $2 - name of file to download.
# $3 - download directory
DownloadFileviaHTTP(){
export http_proxy=http://proxy-us.intel.com:911
`wget http://update.nai.com/$1/$2 -P $3` || return 1
return 0
}
...
DownloadFileviaHTTP "products/commonupdater" "avvdat.ini" "$tmp_dir" \
 || exit_error "downloading avvdat.ini"
update_ini="$tmp_dir/avvdat.ini"
# Did we get avvdat.ini?
[ -r "$update_ini" ] || exit_error "unable to get avvdat file"
...
################# Block 4 ###############
# cleaning ini file from dos to unix format
awk '{ sub("\r$", ""); print }' $update_ini > "$tmp_dir/clean_avvdat.ini" || exit_error "unable to clean $update_ini"
mv "$tmp_dir/clean_avvdat.ini" "$update_ini"

ini_section="AVV-ZIP"
INISection=`FindINISection "$ini_section" < $update_ini`
[ -n "$INISection" ] \
 || exit_error "unable to get section $ini_section from $update_ini"
...
[ -n "$major_ver" -a -n "$minor_ver" -a -n "$file_name" \
 ...
 || exit_error "avvdat: '[$ini_section]' has incomplete data"
 
 
[[ ( $current_major -lt $major_ver ) || ( \
 "$current_major" -eq "$major_ver" && \
...
if [ -n "$perform_update" ] ; then
 ...
 DownloadFileviaHTTP "/products/commonupdater/$file_path" "$file_name" "$tmp_dir" \
 || exit_error "downloading '$file_name'"
